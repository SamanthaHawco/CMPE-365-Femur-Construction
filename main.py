...
def buildTriangles( slice0, slice1 ):
    ...
    #closestPair = [ver1, ver2, dist]    array with three elements, the two vertexes and the distance between them
    closestPair = [slice0.verts[0],slice1.verts[0],calcDistance(slice0.verts[0], slice1.verts[0])]  #start with first points in each slice
    #serch the who slices for shortest distance and save (basic min value search)
    for v0 in slice0.verts:
        for v1 in slice1.verts:
            distance = calcDistance(v0, v1)
            if distance < closestPair[2]:
                closestPair = [v0, v1, distance]
    ...
    permutation0 = [i for i in slice0.verts[slice0.verts.index(closestPair[0]):]] #for vertices in slice0
    permutation1 = [i for i in slice1.verts[slice1.verts.index(closestPair[1]):]] #for vertices in slice1

    for j in slice0.verts[:slice0.verts.index(closestPair[0])]:
        permutation0.append(j)
    for j in slice1.verts[:slice1.verts.index(closestPair[1])]:
        permutation1.append(j)

    permutation0.append(closestPair[0])
    permutation1.append(closestPair[1])
    ...
    minArea = []
    minDir  = []
    for r in range(len(permutation1)):  #add first dimension of array
        minDir.append([])
        minArea.append([])
        for c in range(len(permutation0)):  #add second dimension of array
            minDir[r].append([])
            minArea[r].append([])
    ...
    #initialize row 0 with <- characters
    for c in range(1, len(permutation0)):
        minDir[0][c] = "<-"
        minArea[0][c] = minArea[0][c-1] + triangleArea(permutation1[0].coords, permutation0[c].coords, permutation0[c-1].coords)

    # Fill in col 0 of minArea and minDir, since it's a special case as there's no col -1
    #
    # [2 marks]

    #initialize column 0 with | characters
    for r in range(1, len(permutation1)):
        minDir[r][0] = "|"
        minArea[r][0] = minArea[r-1][0] + triangleArea(permutation0[0].coords, permutation1[r].coords,permutation1[r - 1].coords)
    ...
    for r in range(1, len(permutation1)):
        for c in range(1, len(permutation0)):
            left = minArea[r][c - 1] + triangleArea(permutation1[r].coords, permutation0[c].coords, permutation0[c - 1].coords) #find value of one to the left
            up = minArea[r - 1][c] + triangleArea(permutation0[c].coords, permutation1[r].coords, permutation1[r - 1].coords)   #find value of one up

            if left <= up:                #if the element above [r][c] is greater than or equal to  the element to the left
                minDir[r][c] = "<-"
                minArea[r][c] = left
            elif up < left:                # if the element to above is less than the element to the left
                minDir[r][c] = "|"
                minArea[r][c] = up
    ...
    #     print(r)
    #     for c in range(len(minArea[r])):
    #         print(minDir[r][c],round(minArea[r][c]))
    #     print('\n')
    #
    ...
    #set max length to look through
    x = len(permutation0)-1
    y = len(permutation1)-1

    #loop until x and y are zero
    while(x,y)!=(0,0):
        if minDir[y][x] == "<-":        #if previous column then, add triangle and move to previous column
            triangles.append(Triangle([permutation0[x], permutation1[y], permutation0[x - 1]]))
            x -= 1
        elif minDir[y][x] == "|":       #if previous row the, add triangle and move to previous row
            triangles.append(Triangle([permutation0[x], permutation1[y], permutation1[y-1]]))
            y -= 1
        elif minDir[y][x] == ".":       # if this character occurs, it means you are at the start and it is done
            print("DONE!")
...
def calcDistance(v1, v2):
    distance = math.sqrt((v1.coords[0] - v2.coords[0])**2 + (v1.coords[1] - v2.coords[1])**2 + (v1.coords[2] - v2.coords[2])**2)
    return distance
...
def main():
    ...
    sys.argv.append("femurSlices.dat")
    #sys.argv.append("testSlices.dat")
